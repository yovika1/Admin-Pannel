...
export const ProductList = () => {
  ...
  const handleDelete = async () => {
    try {
      ...
      console.log("product delete sucessfullly", respone.data);
      closeModal();
      getProduct();
  ...
  const handleSearch = async() =>{
    try {
      const searchRes = await axios.get(`http://localhost:8000/searchData?query=${searchQuery}`);
      setFilteredProducts(searchRes?.data?.results)
  ...
  return (
    ...
    <div>
      {!expand ? (
        ...
        <input
          ...
          className="absolute top-24 right-0 z-10 transform -translate-y-1/2 bg-white px-4 py-1 rounded-md border border-gray-300"
        />
      )}
    </div>
    ...
    <div className="ml-64">
      ...
      {emptySpace && <div className=" text-zinc-800 bg-zinc-200 flex justify-center items-center font-semibold h-80 text-lg">product are not added yet</div>}
      </div>
      <DeleteModal
        isOpen={modalIsOpen}
        onClose={closeModal}
